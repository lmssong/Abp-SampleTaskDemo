...
using SamleTaskDemo.Migrations.SeedData;
using SamleTaskDemo.MultiTenancy;
using SamleTaskDemo.Users;
...
{
    public abstract class SamleTaskDemoTestBase : AbpIntegratedTestBase<SamleTaskDemoTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected SamleTaskDemoTestBase()
        {
        ...
        protected void UsingDbContext(Action<SamleTaskDemoDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SamleTaskDemoDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SamleTaskDemoDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SamleTaskDemoDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SamleTaskDemoDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SamleTaskDemoDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SamleTaskDemoDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SamleTaskDemoDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SamleTaskDemoDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SamleTaskDemoDbContext, Task<T>> func)
        {
            T result;
